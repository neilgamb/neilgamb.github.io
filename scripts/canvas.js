...
function floatySpace() {
  ...
  space = new CanvasSpace("canvas", "#292b30" ).display();
  var form = new Form( space );

  // Elements
  var pts = [];
  var center = space.size.$divide(1.5);
  ...
  space.add({
    animate: function(time, fps, context) {
      ...
      for (var i=0; i<pts.length; i++) {
        ...
        pt.rotate2D( Const.one_degree / 60, center);
        form.stroke( false ).fill( colors[i % 3] ).point(pt, 0);

        // get line from pt to the mouse line
        // var ln = new Line( pt ).to( line.getPerpendicularFromPoint(pt));
        var ln = new Line( pt ).to( line.getDistanceFromPoint(pt));
